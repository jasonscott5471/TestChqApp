package cheque.core.repositories.jpa;
...
public class JpaFYBCHKSRepo implements FYBCHKSRepo {
   ...
   public List<FYBCHKS>findAllFYBCHKSs() {
        ...
        /*existing code decided to change see below
        Query query = em.createQuery("SELECT b from FYBCHKS b");
        return query.getResultList();

         */

        //Oct 6, 2016 decided to change this. Intead of just calling a query
        //I want to fire a procedure that loads new data all the time

        StoredProcedureQuery query = em.createStoredProcedureQuery("INSINTOFYBCHKS");
        query.executeUpdate();


        //get the results
        //Query query2 = em.createQuery("SELECT b from FYBCHKS b ORDER BY b.FYBCHKS_VEND_FULL_NAME ");
       // Query query2 = em.createQuery("SELECT b from FYBCHKS b where trim(b.FYBCHKS_DESCRIPTION) not LIKE '%Printed%' ORDER BY b.FYBCHKS_VEND_FULL_NAME");
       // Query query2 = em.createQuery("SELECT b from FYBCHKS b where trim(b.FYBCHKS_DESCRIPTION) not in ('Printed') ORDER BY b.FYBCHKS_VEND_FULL_NAME");
        Query query2 = em.createQuery("SELECT b from FYBCHKS b where b.FYBCHKS_INVH_CODE not in (SELECT distinct c.FYBCHKS_INVH_CODE from FYBCHKS c where c.FYBCHKS_DESCRIPTION like ('%Printed%')) ORDER BY b.FYBCHKS_VEND_FULL_NAME");
